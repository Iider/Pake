...
document.addEventListener("DOMContentLoaded", () => {
  if (!window.pakeConfig || !/^https?:\/\/127\.0\.0\.1/.test(window.pakeConfig.url || "")) return;
  if (!window.__TAURI__ || !window.__TAURI__.core) return;

  const LABEL = "在新窗口中打开";
  const COPY_LABEL = /^(Copy Session ID|复制 Session ID)/;
  const SESSION_ID = /^session_[0-9a-f-]+$/i;
  const ICON = "M10 6v2H5v11h11v-5h2v6a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1V7a1 1 0 0 1 1-1zm11-3v8h-2V6.413l-7.793 7.794l-1.414-1.414L17.585 5H13V3z";
  ...
  function makeItem(copyItem) {
    ...
    while (textNode && !COPY_LABEL.test((textNode.nodeValue || "").replace(/\s+/g, " ").trim())) {
  ...
  function findCopyItem(menu) {
    for (const node of menu.querySelectorAll("[role=button], button, [role=menuitem], div")) {
  ...
  function sessionIdFromContextTarget() {
    for (let element = contextTarget; element; element = element.parentElement) {
      for (let component = element.__vueParentComponent; component; component = component.parent) {
        ...
        const id = candidates.find((value) => typeof value === "string" && SESSION_ID.test(value));
        if (id) return id;
      }
    }
    return "";
  ...
  async function sessionIdFromCopyAction(copyItem) {
    ...
    try {
      Object.defineProperty(clipboard, "writeText", {
        configurable: true,
        value: async (text) => {
          if (typeof text === "string" && SESSION_ID.test(text.trim())) captured = text.trim();
        },
      });
      copyItem.click();
  ...
  async function openSession(copyItem) {
    const id = sessionIdFromContextTarget() || (await sessionIdFromCopyAction(copyItem));
    if (!id) {
      if (window.pakeToast) window.pakeToast("无法获取会话 ID");
      return;
    }
    try {
      await window.__TAURI__.core.invoke("open_session_window", {
        url: window.location.origin + "/sessions/" + encodeURIComponent(id),
      });
    } catch (error) {
      if (window.pakeToast) window.pakeToast("打开新窗口失败");
